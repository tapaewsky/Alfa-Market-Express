...
struct PhoneNumberView: View {
    ...
    private var getCodeButton: some View {
        Button(action: {
            if phoneNumberForServer.count == 11 {
                viewModel.sendCode(phoneNumber: phoneNumberForServer) { success, _ in
                    if success {
                        isCodeSent = true
                    }
                }
    ...
    private var navigationLink: some View {
        NavigationLink(destination: VerificationCodeView(viewModel: MainViewModel(), phoneNumber: phoneNumberForServer),
                       isActive: $isCodeSent)
        {
            EmptyView()
        }
