...
struct CartMainView: View {
    ...
    private var header: some View {
        HStack {
            if isSelectionMode {
                Text("Выбрано: \(viewModel.cartViewModel.selectedProducts.filter(\.value).count)")
                    .font(.headline)
            } else {
                HStack {
    ...
    private func cartItemRow(for cartProduct: CartProduct) -> some View {
        let isSelected = Binding<Bool>(get: { viewModel.cartViewModel.selectedProducts[cartProduct.id] ?? false },
                                       set: { newValue in
        ...
        return ZStack {
            HStack {
                ...
                CartItemView(cartProduct: cartProduct,
                             viewModel: viewModel,
    ...
    private func selectedOrAllProducts() -> [Product] {
        ...
        return selectedProducts.isEmpty ? viewModel.cartViewModel.cartProduct.map(\.product) : selectedProducts.map(\.product)
    }
    
    private func toggleSelectionMode() {
        isSelectionMode.toggle()
        if isSelectionMode {
            let selectedCount = viewModel.cartViewModel.selectedProducts.filter(\.value).count
            if selectedCount == 0 {
                totalPrice = 0
            }
        } else {
    ...
    func calculateTotalPrice() -> Double {
        viewModel.cartViewModel.cartProduct.reduce(0) { $0 + $1.getTotalPrice
        }
    }
