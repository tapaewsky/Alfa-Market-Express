...
class ProductViewModel: ObservableObject {
    ...
    // Метод для сброса данных и пагинации
    func resetData() {
        products.removeAll()  // Очищаем список продуктов
        baseURL = "http://95.174.90.162:60/api/products/"  // Сбрасываем URL для пагинации
    }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            do {
                ...
                DispatchQueue.main.async { [weak self] in
                    ...
                    self.products.append(contentsOf: response.results)
                    self.baseURL = response.next  // Следующий URL для пагинации
                    print("Следующий URL: \(self.baseURL ?? "nil")")
